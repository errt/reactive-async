...
abstract class AbstractIFDSAnalysis[DataFlowFact](parallelism: Int, scheduling: SchedulingStrategy)(implicit project: Project[_]) {
   ...
   */
  def process(
    initialWorklist: mutable.Queue[(BasicBlock, Set[DataFlowFact], Option[Int], Option[Set[Method]], Option[DataFlowFact])])(implicit state: State): Unit = {
  ...
  def mergeMaps[S, T](map1: Map[S, Set[T]], map2: Map[S, Set[T]]): Map[S, Set[T]] = {
    var result = map1
    for ((key, values) ← map2) {
      result = result.updated(key, values ++ result.getOrElse(key, Set.empty))
    }
    result
  }
  ...
  def createResult()(implicit state: State): Outcome[IFDSProperty[DataFlowFact]] = {
    ...
    // at this point all dependees have been collected
    // we can add when-callback for all dependees
    // what about already existing deps? → RA ignores them anyway
  ...
  def getExits(
    method: Method,
    callBB: BasicBlock,
    callIndex: Int): Set[Statement] = {
    val result = exits.get(method)

    if (result == null) {
      // get a default TAC
      val c = tacProvider(method)
      val code = c.stmts
      val cfg = c.cfg
